...
void	color_put(t_data *data, int iteration, int largeur, int hauteur)
{
	...
	if (iteration == MAX_ITERATION)
		my_mlx_pixel_put(data->img1.img, largeur, hauteur, 0xFF000000);
	else
	{
		color.t = 256;
		...
		color.g = 0;
		color.b = 0;
		my_mlx_pixel_put(data->img1.img, largeur, hauteur, makecolor(color.t, color.r, 0, 0));
	}
...
void	mandelbrot(t_data *data)
{
	...
	while (hauteur <= data->height)
	{
		...
		{
			color = ft_formule_mandelbrot(largeur / 400 + p.x1, hauteur / 400 + p.y1);
